...
import { downloadFile, getPublicFiles, getUserFiles, removeFile, uploadFile } from "../../../../common/api";
        ...
        <>
            <div className={className}>
                <div className="relative w-8/12 h-10 bg-yellow-600 rounded-t-md flex items-center p-2">
                    ...
                    <button onClick={() => {if(file){uploadFile(setShowLoginPopUp, file); (CONFIG.tags.space == "USER" ? getUserFiles(setShowLoginPopUp, setFiles) : getPublicFiles(setFiles))}}} className="bg-orange-500 p-1 rounded-md text-white font-bold">Upload!</button>
                ...
                <div className="relative w-8/12 h-full bg-yellow-500 flex items-center p-2 flex-col gap-1">
                    {files.map(file => (
                        <div key={file.fileId} className="relative w-10/12 min-h-20 h-20 bg-orange-600 rounded-md p-2">
                            ...
                            {
                                CONFIG.tags.space == "USER" && <button onClick={() => {removeFile(setShowLoginPopUp, file.fileId); getUserFiles(setShowLoginPopUp, setFiles)}} className="absolute right-2 bottom-2 bg-red-500 bg-opacity-50 p-1 rounded-md text-white font-bold">Remove</button>
                            }
                        </div>
                    ))}
                </div>
